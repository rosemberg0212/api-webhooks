...
const getDeal = async (id) => {
  try {
    ...
    const prospecto = await fetch(
      `https://gehsuites.bitrix24.com/rest/14/${api_key}/crm.deal.get.json?ID=${id}`,
      requestOptions
    );
    if (prospecto.ok) {
...
const getContact = async (id) => {
  try {
    ...
    const contacto = await fetch(
      `https://gehsuites.bitrix24.com/rest/14/${api_key}/crm.contact.get.json?ID=${id}`,
      requestOptions
    );
    if (contacto.ok) {
...
const getCompany = async (id) => {
  try {
    ...
    const prospecto = await fetch(
      `${URL}/${api_key}/crm.company.get.json?ID=${id}`,
      requestOptions
    );
    if (prospecto.ok) {
      const result = await prospecto.json();
...
const addDeal = async (hotelID, datos, url) => {
  ...
  const dealData = {
    fields: {
      ...
      UF_CRM_1730321299: Number(datos.valor_consumo), //valor consumo
      UF_CRM_1732038391218: Number(datos.valor_servicios),
      UF_CRM_1718739210: [Number(datos.valor_total_a_pagar)], //valor total
  ...
  try {
    ...
    if (response.ok) {
      ...
      res.json({
        msg: 'error al crear la negociacion, contactar con soporte',
      });
    }
  } catch (error) {
  ...
  try {
    ...
    while (hasMore) {
      const response = await fetch(
        `https://gehsuites.bitrix24.com/rest/14/${api_key}/crm.deal.list.json?FILTER[STAGE_ID]=C6:WON&SELECT[]=TITLE&SELECT[]=UF_CRM_6635150DE95EB&SELECT[]=UF_CRM_1715689729&SELECT[]=UF_CRM_1714769594&SELECT[]=UF_CRM_1743425226608&start=${start}`,
        requestOptions
      );
...
const fetchTimemanStatus = async (userIds) => {
  ...
  try {
    ...
    const statusPromises = userIds.map((userId) => {
      ...
      return fetch(
        `https://gehsuites.bitrix24.com/rest/14/${apiKey}/timeman.status?USER_ID=${userId}&timestamp=${timestamp}`
      )
...
const enviarMensajeBitrix = async (userId, mensaje) => {
  ...
  try {
    const response = await fetch(
      `https://gehsuites.bitrix24.com/rest/14/${apiKey}/im.message.add`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
const getListPasadias = async () => {
  try {
    ...
    const response = await fetch(
      `${URL}/${apiKey}/crm.deal.list.json?FILTER[STAGE_ID]=C28:NEW&SELECT[]=TITLE&SELECT[]=UF_CRM_1714769540&SELECT[]=UF_CRM_1715802347&SELECT[]=UF_CRM_1718392639543&SELECT[]=CONTACT_ID`,
      requestOptions
    );
...
const getListContact = async () => {
  try {
    ...
    const raw = JSON.stringify({
      ...
      select: ["ID", "NAME", "SECOND_NAME", "LAST_NAME", "EMAIL"],
      start: 0,
    });
    ...
    const response = await fetch(
      `${URL}/${apiKey}/crm.contact.list.json`,
      requestOptions
    );
    if (response.ok) {
      const data = await response.json();
...
const updateDeal = async (data) => {
  ...
  try {
    ...
    if (respuesta.ok) {
      ...
      console.error(
        "Error al actualizar la negociación:",
        resultado.error_description || resultado
      );
    }
  } catch (error) {
