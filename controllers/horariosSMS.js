    ...
    try {
        if (response.ok) {
            ...
            function obtenerDescripcionTurno(turno) {
                ...
                // if (filtroDesc) {
                //     return `- El día *${turno.title
                //         }* su hora de entrada es a las *${filtroDesc.map(
                //             (des) => des.column_values[0].text
                //         )}*, su hora de salida es a las *${filtroDesc.map(
                //             (des) => des.column_values[1].text
                //         )}*, el hotel donde le toca laborar es *${filtroDesc.map(
                //             (des) => des.column_values[2].text
                //         )}* y sus tareas para este día son las de: *${filtroDesc.map(
                //             (des) => des.column_values[3].text
                //         )}*.`;
                // } else if (novedades) {
                ...
                const conca = filtroDesc.map(obj => {
                    if(obj.name == "Novedad 1" || obj.name == "Novedad 2" || obj.name == "Novedad 3" || obj.name == "Novedad 4" || obj.name == "Novedad 5"){
...
const enviarHorariosWindsorMail = async (req, res) => {
    const challenge = req.body.challenge;
    res.send({ challenge });
    const apikey = process.env.APIKEY_MONDAY;
    // const datosTurnos = await traerTurnosWindsor()
    ...
    try {
        if (response.ok) {
            ...
            function obtenerDescripcionTurno(turno) {
                const descripcion = datosTurnos.filter((tur) => tur.name == turno.text);
                const filtroDesc = descripcion.map((obj) => {
                    const columF = obj.column_values.filter(
                        (o) => o.id !== "subelementos"
                    );
                ...
                // console.log(filtroDesc)
                // console.log(descripcion[0].column_values)
                const novedades = datosTurnos.filter(
                    (tur) =>
                        tur.name == "Novedad 1" || "Novedad 2" || "Novedad 3" || "Novedad 4"
                );
                // if (filtroDesc) {
                //     return `- El día ${turno.title
                //         } su hora de entrada es a las ${filtroDesc.map(
                ...
                //             (des) => des.column_values[3].text
                //         )}.`;
                // } else if (filtroDesc.map(obj => obj.name == 'Novedad 1')) {
                //     return `No se encontró una descripción para el código de turno ${turno.title}.`;
                // }
